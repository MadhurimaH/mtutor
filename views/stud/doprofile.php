...
//include_once ('../../models/stud.php');
	...
	if(!empty($value)) {
		...
		//TODO: call StudModel api here ?
/* 		try {
	 		if(StudModel::doprofileupdate($pk, $name, $value)) {
				//Record updated
			} else {
				//DB error
			}
		...
		*/
		...
		if(strpos($name,'qid:') === false) {
			$query = 'UPDATE stud SET '.$name.'= :value WHERE tid= :pk';
			...
			$query = 'SELECT xml_filepath FROM stud_qual_detail WHERE '.$col_name.' IN (SELECT '.$col_name.' from stud WHERE tid= :pk)';
